...
ipw.lc.semipara<-function(samp.data,data,fml,fml2, n.beta, n.gamma, design.mat, xmat,
  ...
  while(keep.going==1){
    ...
    if (length(colnames(mf))>1){
      termnames <- c()
      for (varnum in 4:length(all.vars(fml))){
        if(!(colnames(mf)[(varnum-2)] %in% colnames(design.mat)) & is.factor(mf[,(varnum-2)])){
          if (eval(parse(text=paste0("is.factor(data$",all.vars(fml)[varnum],")")))){
            termnames <- c(termnames,paste0(all.vars(fml)[varnum],"=",eval(parse(text=paste0("levels(data$",all.vars(fml)[varnum],")[-1]")))))
          }
          else {
            termnames <- c(termnames,paste0(all.vars(fml)[varnum],"=",levels(mf[,(varnum-2)])[-1]))
          }
        }
      }
      colnames(design.mat)[2:length(colnames(design.mat))][!(colnames(design.mat)[2:length(colnames(design.mat))] %in% colnames(mf))] <- termnames
    }
    ...
    if(is.null(fml2)){
      ...
      if (length(colnames(mf2))>1){
        termnames <- c()
        for (varnum in 4:length(all.vars(fml2))){
          if(!(colnames(mf2)[(varnum-2)] %in% colnames(xmat)) & is.factor(mf2[,(varnum-2)])){
            if (eval(parse(text=paste0("is.factor(data$",all.vars(fml2)[varnum],")")))){
              termnames <- c(termnames,paste0(all.vars(fml2)[varnum],"=",eval(parse(text=paste0("levels(data$",all.vars(fml2)[varnum],")[-1]")))))
            }
            else {
              termnames <- c(termnames,paste0(all.vars(fml2)[varnum],"=",levels(mf2[,(varnum-2)])[-1]))
            }
          }
        }
        colnames(xmat)[2:length(colnames(xmat))][!(colnames(xmat)[2:length(colnames(xmat))] %in% colnames(mf2))] <- termnames
      }
      if(ncol(xmat)>1){
...
#'
#'
#' @import optimx survival fdrtool interval Icens
#'
#' @section Warning:
